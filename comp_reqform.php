...
<html>
	...
	<body>
		...
		<div class="navbar navbar-inverse navbar-static-top">
			<div class="container">
				<a href="http://localhost/minor/index2.php" class="navbar-brand">Travel4Corporate</a>
				...
				<div class="collapse navbar-collapse navHeaderCollapse">
					<ul class="nav navbar-nav navbar-right">
						<li><a href="http://localhost/minor/index2.php">Home</a></li>
						<li class="dropdown">
						...
						<li class="active"><a href="http://localhost/minor/comp_reqform.php">Requistion Form </a></li>
					</ul>
				</div>
			</div>
		</div> 
		...
		<div class="container">
			...
			<form action="http://localhost/minor/comp_reqform.php" method="POST" class="form-horizontal" role="form">
				  ...
				  <div class="form-group">
					<label for="Noindiv" class="col-sm-2 control-label">Group Strength</label>
					<div class="col-sm-5">
					  <input type="text" class="form-control" id="Noindiv" name="noindiv" placeholder="Group Strength">
					</div>
				  </div>
				  <div class="form-group">
			...
			if(isset($_POST['destination']) && isset($_POST['duration']) && isset($_POST['startdate'])&& isset($_POST['noindiv']) && isset($_POST['hoteltype']) && isset($_POST['travelmode'])&& isset($_POST['budget']))
			{
				...
				if(!empty($destination) && !empty($duration) && !empty($startdate)&& !empty($noindiv) && !empty($hoteltype)&& !empty($travelmode) && !empty($budget))
				{
					$query="INSERT INTO `package` VALUES(DEFAULT,'".mysql_real_escape_string($destination)."','".mysql_real_escape_string($duration)."','".mysql_real_escape_string($startdate)."','".mysql_real_escape_string($noindiv)."','".mysql_real_escape_string($hoteltype)."','".mysql_real_escape_string($travelmode)."','".mysql_real_escape_string($budget)."','".mysql_real_escape_string($custid)."')";
								...
								{	
									$query1="SELECT `manager_email` FROM `companylogin` WHERE `cid`='$custid'";
									if(@$query1_run=mysql_query($query1))
									{
										$header=mysql_result($query1_run,0,'manager_email');
									}
