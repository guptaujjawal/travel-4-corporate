	...
	<body>
		...
		<?php
					if(isset($_POST['username']) && isset($_POST['password']))
					{
						...
						{	
							$query="SELECT `id` FROM `logindetail` WHERE `username` ='".mysql_real_escape_string($username)."' AND `password` ='".mysql_real_escape_string($password_hash)."'";
							if($query_run=mysql_query($query))
							{
								...
								else if($query_num_rows==1)
								{
									...
									header('Location:http://localhost/minor/index1.php');
								}
							}
						}
						else
		...
		<div class="row">
			...
			<h3>Don't have a account? Register with us to explore new oppurtunities!</h3> 
				<a href="http://localhost/minor/register.php" data-toggle="modal" class="btn btn-success">Register</a>
			</div>
		</div>
		</div>
